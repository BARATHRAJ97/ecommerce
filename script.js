...
let cartItems = Number(localStorage.getItem("aravindCartItems") || 0);
let selectedTheme = localStorage.getItem("aravindCartTheme") || "light";
...
if (themeToggle) {
  themeToggle.addEventListener("click", () => {
    ...
    localStorage.setItem("aravindCartTheme", selectedTheme);
    applyTheme(selectedTheme);
  });
}
...
document.querySelectorAll(".add-cart").forEach((button) => {
  button.addEventListener("click", () => {
    cartItems += 1;
    localStorage.setItem("aravindCartItems", String(cartItems));
